...
class PatchMerging2D_sentence(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False, device='cuda')
...
class PatchMerging2D_word(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, stride=2, act_layer=nn.GELU):
        ...
        self.norm = nn.LayerNorm(dim_in, device='cuda')
...
class Stem(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, outer_dim=768, inner_dim=24): # outer_dim=16, inner_dim=4
        ...
        self.common_conv = nn.Sequential(
            nn.Conv2d(in_chans, inner_dim*2, 3, stride=2, padding=1, device='cuda'),
            nn.BatchNorm2d(inner_dim*2, device='cuda'),
            nn.ReLU(inplace=True),
        )
        self.inner_convs = nn.Sequential(
            nn.Conv2d(inner_dim*2, inner_dim, 3, stride=1, padding=1, device='cuda'),
            nn.BatchNorm2d(inner_dim, device='cuda'),
            nn.ReLU(inplace=False),
        )
        self.outer_convs = nn.Sequential(
            nn.Conv2d(inner_dim*2, inner_dim*4, 3, stride=2, padding=1, device='cuda'),
            nn.BatchNorm2d(inner_dim*4, device='cuda'),
            nn.ReLU(inplace=True),
            nn.Conv2d(inner_dim*4, inner_dim*8, 3, stride=2, padding=1, device='cuda'),
            nn.BatchNorm2d(inner_dim*8, device='cuda'),
            nn.ReLU(inplace=True),
            nn.Conv2d(inner_dim*8, outer_dim, 3, stride=1, padding=1, device='cuda'),
            nn.BatchNorm2d(outer_dim, device='cuda'),
            nn.ReLU(inplace=False),
        )
...
class UpsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.transposed_conv = nn.ConvTranspose2d(
            in_channels, out_channels, kernel_size=2, stride=2, padding=0, device='cuda'
        )
        # 批量归一化
        self.batch_norm1 = nn.BatchNorm2d(out_channels, device='cuda')
        # GeLU 激活函数
        self.gelu1 = nn.GELU()
        # 步长为1的3x3卷积
        self.conv = nn.Conv2d(
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, downsample):
        ...
        self.body = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, stride, 1, bias=False, device='cuda'),
        ...
        if downsample:
            self.downsample = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, 1, stride, 0, bias=False, device='cuda'),
                nn.BatchNorm2d(out_channels, device='cuda'),
            )
        else:
            self.downsample = nn.Sequential()
...
class _FCNHead(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_channels, inter_channels, 3, 1, 1, bias=False, device='cuda'),
            nn.BatchNorm2d(inter_channels, device='cuda'),
            nn.ReLU(True),
            nn.Dropout(0.1),
            nn.Conv2d(inter_channels, out_channels, 1, 1, 0, device='cuda')
        )
